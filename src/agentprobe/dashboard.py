"""AgentProbe dashboard. Explains, in plain terms, why the path matters.
...
st.title("AgentProbe")
st.subheader("An agent can return the right answer while taking a broken path. AgentProbe measures the path.")
st.write(
    "A pass or fail check only asks: was the final answer correct? "
    "That hides wasted steps, repeated loops, and failed tool calls. "
    "AgentProbe scores the whole trajectory, so an agent that stumbles to the right answer no longer looks perfect."
)

st.divider()

# ---- a real example, the core teaching moment ----
st.header("See it once, and you get it")
...
if not RESULTS_PATH.exists():
    st.warning("No results.json found. Run: uv run python -m agentprobe.benchmark --runs 3")
    st.stop()

data = json.loads(RESULTS_PATH.read_text())
df = pd.DataFrame(data["models"]).set_index("model").sort_values("avg_traj", ascending=False)

st.header("Which models take the cleanest path?")
